...
class Dropped(Entity):
    ...
    def update(self,dt):
        if self.falling():
            self.move(0,64*dt)
    def isdel(self):
        return abs(entities[0].x-self.x)<2 and abs(entities[0].y-self.y)<2
    def delled(self):
        entities[0].badd(self.iid)
...
wconst=10
edgconst=1
fall=[True,False,False,False,True,True,False]
drop=[0,1,2,3,4,0,5]
put=[0,1,2,3,0,0,0]
iname=[]
